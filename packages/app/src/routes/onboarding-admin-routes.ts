...
function requireAdminOrCoordinator(req: Request, res: Response, next: NextFunction): void {
...
router.post('/applicants/:id/hire', async (req, res) => {
  try {
    ...
    const caregiverRow: Record<string, unknown> = {
      id: newCaregiverId,
      agency_id: agencyId,
    };
    if (hasFirstName) caregiverRow.first_name = applicant.firstName;
    if (hasLastName) caregiverRow.last_name = applicant.lastName;
    if (hasEmail) caregiverRow.email = applicant.email;
    if (hasPhone && applicant.phone) caregiverRow.phone = applicant.phone;
    if (hasStatus) caregiverRow.status = 'active';
    if (hasHiredAt) caregiverRow.hired_at = now;
