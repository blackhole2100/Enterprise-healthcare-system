...
function platformToken(): string {
  return jwt.sign(
    { sub: 'platform-superadmin', scope: 'platform', username: USERNAME },
...
describe('super-admin token-gated actions', () => {
  ...
  it('returns platform stats with a valid token', async () => {
    ...
    vi.spyOn(core, 'PlatformAdminRepository').mockImplementation(() => ({ getPlatformStats } as any));
    const res = await request(createApp())
  ...
  it('returns the global activity feed', async () => {
    ...
    vi.spyOn(core, 'PlatformAdminRepository').mockImplementation(() => ({ getRecentActivity } as any));
    const res = await request(createApp())
  ...
  it('returns an agency drill-down detail', async () => {
    ...
    vi.spyOn(core, 'PlatformAdminRepository').mockImplementation(() => ({ getAgencyDetail } as any));
    const res = await request(createApp())
