...
function mockRepos(overrides: RepoOverrides = {}) {
  ...
  vi.spyOn(core, 'ClaimRepository').mockImplementation(() => repo as unknown as core.ClaimRepository);
...
describe('billing claims routes', () => {
  ...
  it('generates claims for an admin and persists them', async () => {
    ...
    expect((repo.createClaims as ReturnType<typeof vi.fn>).mock.calls[0][0]).toHaveLength(1);
  });
  ...
  it('downloads a valid 837P for a claim', async () => {
    mockRepos({
      ...
      getVisitRenderingProviders: vi.fn().mockResolvedValue(
        new Map([['visit-1', { firstName: 'ALEX', lastName: 'SMITH', npi: '1987654321' }]]),
      ),
    });
