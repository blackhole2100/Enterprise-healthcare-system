...
function membership(agencyId: string, caregiverId: string, overrides: Partial<core.UserAgencyMembership> = {}): core.UserAgencyMembership {
  return {
    userId,
    agencyId,
  ...
  it('lists active memberships with the current agency id', async () => {
    vi.spyOn(core, 'UserAgencyRepository').mockImplementation(
      () =>
        ...
        }) as unknown as core.UserAgencyRepository
    );
...
describe('POST /auth/mobile/switch-agency', () => {
  it('issues a token re-scoped to the target agency from the membership row', async () => {
    ...
    vi.spyOn(core, 'UserAgencyRepository').mockImplementation(
      () => ({ findMembership }) as unknown as core.UserAgencyRepository
    );
    ...
    const claims = jwt.decode(response.body.token) as Record<string, unknown>;
  ...
  it('denies switching to an agency without an active membership', async () => {
    vi.spyOn(core, 'UserAgencyRepository').mockImplementation(
      () =>
        ({ findMembership: vi.fn().mockResolvedValue(undefined) }) as unknown as core.UserAgencyRepository
    );
