...
function stubStorage() {
  ...
  vi.spyOn(S3StorageService.prototype, 'getSignedDownloadUrl').mockResolvedValue(
    'https://signed.example/url',
  );
}

function stubClientInAgency(belongs: boolean) {
  vi.spyOn(core, 'ClientRepository').mockImplementation(
    () => ({ clientBelongsToAgency: vi.fn().mockResolvedValue(belongs) }) as any,
  );
}
...
describe('documents routes, prior-auth upload', () => {
  ...
  it('uploads, sanitizes the filename into a tenant-scoped key, and audits', async () => {
    ...
    const uploadArg = (S3StorageService.prototype.uploadDocument as any).mock.calls[0][0];
