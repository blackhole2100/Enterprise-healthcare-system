...
function emptyPartialFor(agencyId: string): PartialHhaexchangeConfig {
  return {
    agencyId,
    agencyTaxId: null,
    hhaProviderId: null,
...
function mergeConfig(
  current: PartialHhaexchangeConfig,
  update: HhaexchangeConfigUpdate,
): PartialHhaexchangeConfig {
  return {
    agencyId: current.agencyId,
    agencyTaxId: update.agencyTaxId !== undefined ? update.agencyTaxId : current.agencyTaxId,
    hhaProviderId:
      update.hhaProviderId !== undefined ? update.hhaProviderId : current.hhaProviderId,
 ...
 *  null. */
function validateForEnable(next: PartialHhaexchangeConfig): string | null {
    ...
    try {
      ...
      try {
        ...
        process.stderr.write(
          `[audit-write-failed] agency.evv-config.changed (hhaexchange) agency=${req.auth.agencyId} ` +
...
function redactForAudit(c: PartialHhaexchangeConfig): Record<string, unknown> {
  return {
