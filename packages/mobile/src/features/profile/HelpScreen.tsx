...
const HELP_SECTIONS: HelpSection[] = [
  ...
  },
  {
    ...
    image: require('../../../assets/images/help/home.jpg'),
    imageAlt: 'A caregiver showing her client something on a phone',
    blocks: [
      {
        text:
  ...
  },
  {
    ...
    image: require('../../../assets/images/help/schedule.jpg'),
    imageAlt: 'A weekly planner open on a table beside a cup of coffee',
    blocks: [
      {
      ...
      },
      {
        ...
        legend: [
          { color: colors.success, label: 'Visit verified successfully, or a time edit was approved' },
          { color: colors.amber, label: 'Time edit awaiting agency approval' },
          { color: colors.danger, label: 'Visit verification issue detected' },
          { color: colors.slate, label: 'Gray text, the visit has already been billed' },
        ],
      },
    ],
  },
  {
    ...
    image: require('../../../assets/images/help/visits.jpg'),
    imageAlt: 'A home health nurse checking a patient during a visit',
    blocks: [
      {
        text:
  ...
  },
  {
    ...
    image: require('../../../assets/images/help/unscheduled.jpg'),
    imageAlt: 'A caregiver helping a smiling woman up from her chair',
    blocks: [
      {
        text:
  ...
  },
  {
    ...
    image: require('../../../assets/images/help/patients.jpg'),
    imageAlt: 'A clinician talking through care details with a patient at home',
    blocks: [
      {
  ...
  },
  {
    ...
    image: require('../../../assets/images/help/messages.jpg'),
    imageAlt: 'A clinician typing a message on a phone',
    blocks: [
      {
        text:
      ...
      },
      {
        ...
        icon: 'flag-outline',
        legend: [
          { color: colors.danger, label: 'High priority' },
          { color: colors.amber, label: 'Medium priority' },
          { color: colors.brandBlueLight, label: 'Low priority / informational' },
        ],
      },
      {
  ...
  },
  {
    ...
    image: require('../../../assets/images/help/support.jpg'),
    imageAlt: 'A pair of hands resting gently on a windowsill',
    blocks: [
      {
        bullets: [
...
function matchesQuery(section: HelpSection, q: string): boolean {
  ...
  return section.blocks.some(
    (b) =>
      ...
      b.legend?.some((x) => x.label.toLowerCase().includes(needle)),
  );
}
    ...
    <View style={styles.block}>
      {block.heading ? (
        <View style={styles.blockHeadRow}>
          {block.icon ? (
            <View style={[styles.blockHeadIcon, { backgroundColor: `${tint}${alpha.tint}` }]}>
              <Ionicons name={block.icon} size={13} color={tint} />
            </View>
          ) : null}
      ...
      {block.legend ? (
        <View style={styles.legendCard}>
          {block.legend.map((row, i) => (
            <View
              key={row.label}
              style={[styles.legendRow, i > 0 && styles.legendRowDivider]}
            >
    ...
    <View style={styles.container}>
      ...
      <ScrollView contentContainerStyle={styles.scroll} showsVerticalScrollIndicator={false}>
        <View style={styles.hero}>
          <Image
            source={require('../../../assets/images/help/hero.jpg')}
            style={StyleSheet.absoluteFill}
            contentFit="cover"
            transition={250}
            accessible
            accessibilityLabel="A smiling caregiver leaning in close to her client at home"
          />
          <LinearGradient
            colors={['transparent', 'rgba(15,45,82,0.55)', 'rgba(15,45,82,0.92)']}
                  ...
                  {isOpen ? (
                    <View style={styles.sectionBody}>
                      <Image
                        ...
                        transition={250}
                        accessible
                        accessibilityLabel={section.imageAlt}
                      />
...
const styles = StyleSheet.create({
  ...
  hero: {
    height: 190, borderRadius: radii.hero, overflow: 'hidden',
    justifyContent: 'flex-end', backgroundColor: colors.navy,
    ...shadow.card,
  },
  ...
  heroPillText: { ...typography.label, color: colors.onGradient },
  heroTitle: { ...typography.title, color: colors.onGradient, marginTop: 8 },
  heroSub: { fontSize: 12.5, color: colors.onGradientSoft, marginTop: 4, lineHeight: 18 },
  ...
  sectionBody: {
    paddingHorizontal: 16, paddingBottom: 16, paddingTop: 14,
    borderTopWidth: StyleSheet.hairlineWidth, borderTopColor: colors.border,
  },
  sectionImage: {
    width: '100%', height: 136, borderRadius: radii.md, backgroundColor: colors.screenBg,
  },

  block: { marginTop: 12 },
  blockHeadRow: { flexDirection: 'row', alignItems: 'center', gap: 7, marginBottom: 5 },
  blockHeadIcon: {
    width: 24, height: 24, borderRadius: 7, justifyContent: 'center', alignItems: 'center',
  },
  blockHeading: { flex: 1, fontSize: 13.5, fontWeight: '800', color: colors.textPrimary },
  ...
  legendCard: {
    backgroundColor: colors.inputBg, borderRadius: radii.sm, borderWidth: 1, borderColor: colors.border,
    paddingHorizontal: 12, marginTop: 4,
  },
  legendRow: { flexDirection: 'row', alignItems: 'center', gap: 10, paddingVertical: 10 },
  legendRowDivider: { borderTopWidth: StyleSheet.hairlineWidth, borderTopColor: colors.border },
  legendDot: {
    width: 12, height: 12, borderRadius: 6,
    borderWidth: 2, borderColor: colors.cardBg,
    shadowColor: colors.navy, shadowOpacity: 0.2, shadowRadius: 2, shadowOffset: { width: 0, height: 1 },
    elevation: 1,
  },
  legendText: { flex: 1, fontSize: 13, color: colors.textSecondary, lineHeight: 18 },
